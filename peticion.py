...
def main(C,CC,diag):
    ...
    if diag not in leucemias:
        try:
            Quimios = []
            persona = info(C,CC)
            data_med = pd.read_csv('medicamentos_orales.csv')
            medicamentos = list(data_med['descripcion_atc'].unique())
            if persona["data"] != []:
                            ...
                            if order["ordering"] != []:
                                for orden in order["ordering"]:
                                    ...
                                    for medicamento in medicamentos:
                                        if medicamento in orden["sumDesc"]:
                                            if medicamento == "H02AB02":
                                                continue
                                            # codigo = medicamento
                                            codigo = data_med[data_med["descripcion_atc"]==medicamento]["codigo_atc"].values[0]
...
if __name__ == '__main__':
    cedulas = ["8713010","28104919","30723970","72044663","40797605","72044663"]
    for c in cedulas:
        print("\n","--"*50)
